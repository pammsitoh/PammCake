...
exports.handler = async function (argv) {
    ...
    const editing_bp_path = path.join(rutaCarpetaUsuario, 'development_behavior_packs', `${config.name}[${config.identifier}] - BP`);
    const editing_rp_path = path.join(rutaCarpetaUsuario, 'development_resource_packs', `${config.name}[${config.identifier}] - RP`);
    ...
    const syncFolders = async (source, target) => {
        try {
            ...
            console.log(`█+█ Carpeta ${source} sincronizada con ${target}`.green);
        } catch (err) {
            console.error(`[!] Error al sincronizar carpetas: ${err.message}`.red);
        }
    };
