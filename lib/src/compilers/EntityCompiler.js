...
class EntityCompiler {
    ...
    loadCompilables(callback) {
        fs.readdir(this.project_path, (err, files) => {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                if (
                    content["minecraft:entity"].description.hasOwnProperty(
                        "modules"
                    )
                ) {
    ...
    compile(file) {
        ...
        modules.forEach((module) => {
            switch (module) {
                case "despawn":
                    content["minecraft:entity"].component_groups[
                        DespawnModule.name
                    ] = DespawnModule.group;
                    content["minecraft:entity"].events[DespawnModule.name] = {
                        add: {
                            component_groups: [DespawnModule.name],
                        },
                    };
                ...
                case "rare_functions":
                    ...
                    for (const key of keys) {
                        ...
                        if (args[0] == "f") {
                            ...
                            delete content["minecraft:entity"].component_groups[
                                key
                            ];
                        }
                    }
